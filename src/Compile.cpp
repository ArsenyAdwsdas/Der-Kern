...
namespace DerKern{
	...
	void Instructionary::compile(){
		...
		auto w=__b.raw;__b.raw=(uint8_t*)(env?env->exeMem.alloc(siz+inliny):ExeMem.alloc(siz+inliny));memcpy(__b.raw,w,siz);free(w);//replace the buffer with allocated executable memory
		co.post();//post-process s---

		if(inliny){co.inlining=0;uint32_t _=__b.count;Instructions::ret.compile(co);siz+=__b.count-_;}//add "ret" if inlining
		__b.size=0;//disable BBuf cleanup(size=0)
		if(env){
			if(siz>__b.count)env->exeMem.shrink(__b.raw,__b.count);
		}else ExeMem.count-=siz-__b.count;
		_compiled=__b.raw;
		printf("Compiled: ",_compiled);for(uint32_t i=0;i<__b.count;i++)printf("%.2x",__b[i]);printf("\n");
	}
	...
	namespace __COMPILE{
		...
		INITIALIZER(init){
			...
			CompileState co(0,&_EVALER);BBuf b;co.b=&b;co.i=0;co.line=0;
			if(!co.compile())throw std::exception();Instructions::ret.compile(co);//compile and add "ret"
			uint32_t siz=b.count;
			auto w=b.raw;_EVALER._compiled=b.raw=(uint8_t*)ExeMem.alloCpy(b.raw,siz);b.size=0;free(w);co.post();//basically a shorter version of a line in Instructionary::compile for the "Environment"-less environment of "global construction"
